    ...
    def __init__(self):
        ...
        self.prompts = {
            ...
            'ask_additional': """✨ 기본적인 내용은 잘 알겠습니다!
                ...
                • 특별히 신경 쓰시는 부분이 있나요? (예: 발열, 배터리, 필기감 등)
...
다시 정리해드릴게요!""",
    ...
    async def _prepare_review_agent_state(self, requirements: str) -> Dict[str, Any]:
        try:
            prompt = f"""
            다음 정보를 바탕으로 태블릿 사용자의 요구사항을 상세하게 분석해주세요.

            분석 시 다음 원칙을 지켜주세요:
            1. 사용자가 직접 언급한 내용은 그대로 포함
            2. 명확하게 연관된 내용만 합리적 수준에서 추론 (예: 게임 용도 → 높은 성능 선호)
            3. 불확실한 내용은 추측하지 말고 생략
            4. 구체적인 수치나 세부 사항은 사용자가 직접 언급한 경우에만 포함
            ...
            {{
                ...
                "주요_관심사": {{
                    "브랜드_선호도": ["선호하는 브랜드들"],
                    "불편사항": ["우려되는 문제점들"],
                    "만족도_중요항목": ["중요하게 생각하는 기능/특성들"]
                }},
                "감성적_요구사항": {{
                    "디자인_선호도": ["선호하는 디자인 특성들"],
                    "가격대_심리": "가격에 대한 태도"
                }},
                "사용자_우려사항": ["구매/사용시 걱정되는 점들"]
            }}
            """
