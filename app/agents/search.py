...
import re
from dataloader import DataLoader, get_searchdata
import difflib
import pandas as pd
import random
globalist=[]
...
def retrun_fail_result():
    ...
    out['raw_meta_data']={}
    out['llm_process_data']['timestamps']="인덱싱 오류로 적합한 데이터 추출 실패"
    out['llm_process_data']['timestampsdiscriptions']="인덱싱 오류로 적합한 데이터 추출 실패"
    out['llm_process_data']['seconds']="인덱싱 오류로 적합한 데이터 추출 실패"
    out['llm_process_data']['descriptions']="인덱싱 오류로 적합한 데이터 추출 실패"
    out['llm_process_data']['codes']="인덱싱 오류로 적합한 데이터 추출 실패"
    out['llm_process_data']['clip']="인덱싱 오류로 적합한 데이터 추출 실패"
    ...
    return out, ['llm_process_data','raw_meta_data'],None
