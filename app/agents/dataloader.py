...
class Dataprocessor:
    def __init__(self, target_dir="youtube", ref_file="YTref.txt",pickle_file="./copydata/data.pkl",mode=None):
        dir=Path("./copydata")
        ...
        dimension = 1536
        self.vectorstore = HDF5VectorDB("data/vector_db.h5", dimension)
        ...
        #self.create_summary_dicts()     ###########################################                    
        if os.path.exists("./copydata/tot_doc_len.pkl"):
            self.tot_doc_len=self.load_data_from_pickle("./copydata/tot_doc_len.pkl")
            log_wrapper("전체 문서 길이 로드 완료")
        if os.path.exists("./copydata/Index_table.pkl"):
            self.Index_table=pd.DataFrame(self.load_data_from_pickle("./copydata/Index_table.pkl"))
            log_wrapper("문서별 태그 정보 로드 완료")
        if os.path.exists("./copydata/heshdict.pkl"):
            self.heshdict=self.load_data_from_pickle("./copydata/heshdict.pkl")
            log_wrapper("해시 딕셔너리 로드 완료")
        elif not self.Index_table.empty:
            self.make_hesh_dict()            
        if os.path.exists("./copydata/keyword_set.pkl"):
            self.keyword_set=set(self.load_data_from_pickle("./copydata/keyword_set.pkl"))
            log_wrapper("전체 태그셋 로드 완료")
        if os.path.exists("./copydata/summary.pkl"):
            self.summary_list=self.load_data_from_pickle("./copydata/summary.pkl")
        ...
        if os.path.exists("./copydata/datelimit.pkl"):
            self.datelimit=self.load_data_from_pickle("./copydata/datelimit.pkl")
    ...
    def set_day_limit(self):
        ...
        self.save_data_to_pickle(self.datelimit,"./copydata/datelimit.pkl")
    ...
    def make_hesh_dict(self):
        ...
        self.save_data_to_pickle(heshdict,"./copydata/heshdict.pkl")
    def load_hesh_dict(self):
        return self.load_data_from_pickle("./copydata/heshdict.pkl")
    def remove_pickle(self,lang_path="./copydata/tot_doc_len.pkl",summary_path="./copydata/summary.pkl",index_table="./copydata/Index_table.pkl",keyword_set="./copydata/keyword_set.pkl"):
    ...
    def load_srt_in_folder(self):
        ...
        텍스트 파일(.csv, .srt)만 로드하고, 로드한 파일의 경로와 내용을 리스트로 반환합니다.
        """
        self.save_data_to_pickle(self.tot_doc_len,"./copydata/tot_doc_len.pkl")
        loaded_files = []
        full_tocken=0
        totn=0
...
#
        ...
        self.save_data_to_pickle(summary_list, "./copydata/summary.pkl")
    ...
    def create_vector_store_active(self, persist_directory="data/vector_db.h5"):
        """
        ...
        dimension = 1536
        vectorstore = HDF5VectorDB("data/vector_db.h5", dimension)
    ...
    def create_qa_chain_from_llm(self, model_name="gpt-4o-mini", temperature=0, persist_directory="data/vector_db.h5"):
        """
        ...
        self.qa=qa
    def create_qa_chain_from_store(self,model_name="gpt-4o-mini", temperature=0, persist_directory="data/vector_db.h5"):
        load_dotenv()
    ...
    #@staticmethod
    #def load_vector_store(persist_directory="data/vector_db.h5"):
    ...
    def setup_tag_table(self):
        ...
        self.save_data_to_pickle(self.Index_table,"./copydata/Index_table.pkl")
        self.save_data_to_pickle(list(self.keyword_set),"./copydata/keyword_set.pkl")
...
class DataExcelerator:
    def __init__(self):
        ...
        self.DataProcessor.create_vector_store_active(persist_directory="data/vector_db.h5")
        log_wrapper("활성 상태")
class Datatagger:
    def __init__(self):
...
def get_searchdata(keyword_list,query):
    ...
    R.DataProcessor.create_qa_chain_from_store(persist_directory="data/vector_db.h5")
...
if __name__ == "__main__":
    ...
    if args.mode=="tag":
        Datatagger()
        dir_flag=Path("./copydata/flag.txt")
        ...
        DataProcessor.create_qa_chain_from_store(persist_directory="data/vector_db.h5")
